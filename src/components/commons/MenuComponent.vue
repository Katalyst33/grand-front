          ...
          <div class="absolute inset-y-0 right-0 flex items-center pr-2 sm:static sm:inset-auto sm:ml-6 sm:pr-0">
            ...
            <div class="hidden sm:block sm:ml-6">
              ...
              <div class="flex space-x-4">
                <router-link v-for="item in navigation" :key="item.name" :to="{name:item.href}" class="text-white menu-links"  active-class="active"  :aria-current="item.current ? 'page' : undefined">{{ item.name }}</router-link>
...
<!--    mobiul menu pages links-->
      <DisclosurePanel class="sm:hidden">
        <div class="px-2 pt-2 pb-3 space-y-1 flex flex-col text-gray-100">
          <router-link v-for="item in navigation" :key="item.name" :to="{name:item.href}" active-class="active"  class=" hover:bg-gray-700 hover:text-white block px-3 py-2 rounded-md text-base font-medium" >{{ item.name }}</router-link>
        </div>
